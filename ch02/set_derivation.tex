...
The pushforward measure $\predictedP$ is characterized by the \emph{predicted density} $\predicted$, which is given by the Radon-Nikodym derivative of $\predictedP$ with respect to measure $\mu_\dspace$ on ${(\dspace, \dborel)}$.
In practice, when $\predictedP$ is absolutely continuous with respect to the $\dimD$--dimensional Lebesgue measure $\mu_\dspace$, we substitute this Lebesgue measure for $\predictedP$.
...
We colloquially refer to the density (given by the Radon-Nikodym derivative $d\dataP / d\mu_\dspace$) as the \emph{observed density}, where $\pmeas$ is taken to be the (Lebesgue) volume measure on ${(\pspace, \pborel)}$.
...
\paramP \left ( \qoi^{-1}(E)\right ) = \dataP(E) \quad \forall \, E \in \dborel.
\end{equation}
