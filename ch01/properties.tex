...
Therefore, either prior beliefs ($\initial$), the model under consideration ($\qoi$), or the description of uncertainty encoded in $\observed$ should be subjected to a critical reevaluation. 
...
\end{equation} 
is a consistent solution to the stochastic inverse problem given in (\eqref{eq:inverse-problem}), and is uniquely defined up to the specification of a prior probability measure $\initialP$ on $(\pspace, \pborel)$.
\end{thm}
...
Corollary~\ref{cor:int} is critical to understanding some significant differences between the classical/statistical Bayesian updated density~\cite{Smith} and the updated density density given by \eqref{eq:update}, which we briefly discuss in Section~\ref{sec:othermethods}.
...
\begin{thm}
$\updatedP$ is stable with respect to perturbations to the observed, $\observedP$.
\label{thm:stableobs}
\end{thm}
...
Given $\initialP$ and $\observedP$, let $\widehat{\initialP}$ be any perturbation to $\initialP$ on $(\pspace, \pborel)$ satisfying \eqref{eq:pred} (the push-forwards of both are dominating measures for the given $\observedP$). 
Let $\updatedP$ and $\widehat{\updatedP}$ denote the consistent solutions associated with $\observedP$ and $\widehat{\observedP}$, respectively. 
...
\begin{thm}
$\updatedP$ is stable with respect to perturbations to the initial, $\initialP$.
\label{thm:stableprior}
\end{thm}

Taken together, these stability results provide assurances that the updated density we obtain is ``accurate'' up to the level of experimental error polluting $\observedP$ and error in incorrectly specifying $\initialP$. 
...
\footnote{
Throughout this work, we treat $\initialP$ as a basic description of feasible inputs to the model.
For simplicity, we often rely on normalized domains to allow for uniform initial densities on unit hypercubes.
This amounts to an implied pre-processsing of $\pspace$ wherein scaling is performed to normalize the parameter domain. 
To this end, units of measurement are effectively removed, and so they are not present on figure labels.
One can quickly make qualitative inferences based on the support of $\updated$ relative to a unit-domain to get a sense for the proportionate reduction of uncertainty.
}.
However, generating samples from $\updated$ requires a numerical approximation to $\predicted$, which introduces additional errors in $\updated$.
If $\widehat{\predicted}$ denotes a computational approximation to the push-forward of the initial density, then the conditional densities from the disintegration theorem are given as
\[
\frac{\widehat{d\PP_{\text{up}, d}}}{d\mu_{\pspace, d}\lam} = \frac{\initial\lam}{ \widehat{\predicted\q} }.
\]
...
As before, we assume the following for the approximation of the push-forward of the initial density $\initial$:
...
We leverage the re-use of samples in the results and examples herein extensively.

In summary, the \textbf{accuracy of the computed updated density relies on the accuracy of the approximation of the push-forward of the initial density.}
Throughout this work, we utilize a Gaussian (KDE), which converges at a rate of $\mathcal{O}(N^{-4/(4+d)})$ in mean-squared error and $\mathcal{O}(N^{-2/(4+d)})$ in $L^1$-error, where $d$ is the dimension of $\dspace$, and $N$ is the number of samples from $\initial$ propagated through $\qoi$.
...
In some cases, when it is possible to specify $\predicted$ analytically, we will do so for purposes of demonstration (especially for comparing numerical solutions to their ``true'' ones). 