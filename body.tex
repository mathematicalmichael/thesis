...
%As discussed in the introduction, the typical Bayesian framework focuses on first modeling epistemic uncertainties in data on a QoI obtained from a true, but unknown, parameter value, which we denote by $\lam_true$.
...
% To build intuition around the fundamental differences of MAP and MUD points, we consider an example where the linear QoI map is defined by $A=\mat{1 & 1}$, i.e., the parameter space is 2-dimensional while the data space is 1-dimensional.
% The parameters in the initial and observed densities are given by
% \[
% 	\initialMean = \mat{0.25 & 0.25}^\top, \initialCov = \mat{1 & -0.25 \\ -0.25 & 0.5}, \ \observedMean=1, \text{ and } \observedCov = \mat{0.25}.
% \]
...
%For under-determined QoI maps, $\lam_true$ is un-recoverable due to the set-valued nature of the set $Q^{-1}(\observedMean)$.
%While the incorporation of prior beliefs through the use of an initial or prior density allows us to determine a single ``best'' estimate of $\lam_true$, this is not necessary.
%For instance, the least squares (LS) solution produces a single point estimate of $\lam_true$ by enforcing a requirement of minimal norm.
...
%Here, we demonstrate how the various estimates of a true parameter $\lam_true$ are impacted by the number of available QoI.%, especially in problems where the dimension of $\pspace$ is high.
%As more QoI are incorporated, the expectation is that the accuracy of $\lam_true$ improves as well.
%In the following example, we demonstrate that the MUD point does indeed perform this way, and compare its ability to approximate $\lam_true$ against two other popular solution methods: the least-squares solution and Tikhonov-regularized solution, i.e. the MAP point.
...
The same distribution is also used for generating the components of the 100-dimensional vectors defining a reference bias vector $\mathbf{b}$ and reference parameter $\lam_true$.
...
%We then attempt to recover $\lam_true$ with the three solution methods: MUD, MAP, and Least Squares, leveraging analytical solutions for all three since we have posed a linear problem under Gaussian assumptions.
...
%The same distribution is used for the components of vector $b$ and $\lam_true$ as well.
...
%We scale the initial covariance $\initialCov$ by a factor $\alpha$ for $\alpha = 0.01, 0.1, 1, 10 $ to simulate varying levels of attachment to the specification of the initial density.
%The results are summarized in Figure~\ref{fig:lin-error}, which shows convergence towards $\lam_true$ for all the problems considered with the exception of several Tikhonov solutions corresponding to strongly-held beliefs in initial assumptions about parameters.
...
%  \item Vertical axes on convergence plots represent the absolute error between $\lam_true$ and $\mudpt$, with bounds chosen to be fixed across plots within a figure.
...
\begin{figure}[htb]
  ...
  {
  \color{blue}{Change "Proportion of Time Elapsed" to "N" and plot vs $N$. Remove the orange scatter points and curves. Change titles to $\mathbb{E}(\abs{\mudpt - \lam_true})$ and $Var(\abs{\mudpt-\lam_true})$.}
  }
  }
...
For 20 different realizations of $N=5, 10, 15, 20, 25, 50, 100, \text{ and } 200$ noisy measurement data, we construct $Q_\text{WME}$ with $u(t;\param)$ replacing $M_j(\param)$ in~\eqref{eq:qoi_WME}, estimate $\mudpt$, and analyze the statistical error in these estimates of $\lam_true$.
