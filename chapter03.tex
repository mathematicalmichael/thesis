...
%
%The measures computed from Algorithm~\ref{alg:inv_density} are defined on a set of samples $S = \sett{\param^{(j)}}{j=1}{N}$ which implicitly define a Voronoi-cell partition $\sett{\VVV^{(j)}}{j=1}{N}$ of the parameter space $\pspace$. 
%We let $\BBB_{\pspace, N}$ denote the \emph{computational algebra} generated by $\sett{\VVV^{(j)}}{j=1}{N}$, i.e., using standard measure theory notation, 
%$$
%	\BBB_{\pspace,N} = \sigma\left(\sett{\VVV^{(j)}}{j=1}{N}\right).
%$$
%Clearly, $\BBB_{\pspace, N}\subset \BBB_\pspace$ and the events $A\in B_{\pspace, N}$ represent the $A\in\BBB_\pspace$ for which we can ``easily'' compute probabilities and make inferences.
%While Algorithm~\ref{alg:inv_density} ultimately defines a probability measure implicitly on $(\pspace,\BBB_\pspace)$, computationally this is almost never done and the measures are only interrogated on the computational algebra associated with the set of samples. 
%
%Different sets $S_k = \sett{\param^{(i)}}{i=1}{N_k}$, where the $\param^{(i)}$'s and $N_k$'s may be completely different for each $k$, will lead to different measures computed from Algorithm~\ref{alg:inv_density}. 
%Each $S_k$ induces a computational algebra which we index using the notation $\BBB_{k}$ for simplicity, where it is understood that $\BBB_k = \BBB_{\pspace, N_k}$. 
%
...
%Let $\mu$ be a measure on $(\pspace, \BBB_\pspace)$, $\sett{\VVV^{(j)}}{j=1}{N}$ be a partition of $\pspace$, and $\BBB_{\pspace, N}$ the computational algebra generated by $\sett{\VVV^{(j)}}{j=1}{N}$. 
%Assume $\mu (\VVV^{(j)}) > 0 \; \forall \; j=1,\hdots, N$. 
%Then, there exists a probability measure $\eta$ on $(\pspace, \BBB_\pspace)$ such that $\eta(A) = \eta_N(A) \; \forall \; A\in\BBB_{\pspace, N}$. 
%\end{lem}
...
%f_N (\param) = \sum_{j=1}^{N} \frac{\eta_N (\VVV^{(j)}) }{\mu (\VVV^{(j)})} \Chi_{\VVV^{(j)}} (\param).
%\end{equation}
%Then, for any $A\in\BBB_\pspace$, define
%\begin{equation}\label{eq:approxmeasure}
%\eta (A) = \int_A f_N (\param) \, d\mu.
%\end{equation}
%We verify that $\eta$ is a probability measure on $(\pspace, \BBB_\pspace)$ and that $\eta(A) = \eta_N(A) \; \forall \; A\in\BBB_{\pspace, N}$ below:
%\begin{itemize}
%\item[(i)][Positive]
%Let $A\in \BBB_{\pspace}$.
%\begin{equation*}
%\begin{split}
%\eta (A) &= \int_A f_N (\param) \, d\mu \\
%&=  \int \Chi_A \sum_{j=1}^{N} \frac{\eta_N (\VVV^{(j)}) }{\mu (\VVV^{(j)})} \Chi_{\VVV^{(j)}} (\param) \, d\mu \\
%&= \sum_{j=1}^{N} \left ( \frac{\eta_N (\VVV^{(j)}) }{\mu (\VVV^{(j)})} \int \Chi_{A\cap\VVV^{(j)}} (\param) \, d\mu \right ) \\
...
%Let $\sett{A_k}{k=1}{\infty} \subset \BBB_{\pspace}$.
%\begin{equation*}
%\begin{split}
...
%
%
%\noindent Finally, let $A\in\BBB_{\pspace, N}\subset \BBB_\pspace$. 
...
%\end{proof}
%
%We note that in practice, $\Chi_{\VVV^{(j)}} (\param)$ requires the use of nearest-neighbor computations, but otherwise evaluation of Eq.~\eqref{eq:finiteradon} is straightforward to compute.
%
%\begin{algorithm}
%\DontPrintSemicolon
...
%\end{algorithm}
%
%Since we now have a way to extend probability measures defined on $(\pspace, \BBB_{\pspace, N})$ to  probability measure on $(\pspace, \BBB_{\pspace})$, we can use simple Monte-Carlo approximation schemes to the Hellinger distance between two probability measures defined on two separate computational algebras. 
