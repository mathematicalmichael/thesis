...
def plot_surface(res, measurements, prefix, lam_true, fsize=32, test=False):
    ...
    for _res in res:
        ...
        if not test: plt.savefig(f'{_prefix}_qoi_response.png', bbox_inches='tight')
        #plt.show()
        ...
        if not test: plt.savefig(f'{_prefix}_sensitivity_qoi.png', bbox_inches='tight')
        #plt.show()
...
#     num_meas_plot_list = [25, 50, 400]
    ...
    #     plt.annotate("$\\downarrow$ Possible Signals", (0,annotate_height), fontsize=fsize)
        ...
        plt.plot(plotting_mesh, plot_model(mud_solutions[0][0]), lw=3, c='xkcd:bright red', alpha=alpha_signal_mudpts, label=f'{len(mud_solutions)} Updated Solutions for S={num_meas_plot}')
