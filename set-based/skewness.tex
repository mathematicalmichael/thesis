In \cite{BGE+15}, the concept of skewness in a QoI map $\qoi$ is introduced, quantified, agind related to the accuracy in solving the stochastic inverse problem with a finite number of samples.
...
For completeness, we define skewness below and refer the interested reader to \cite{BGE+15, Walsh} for more details.
...
In \cite{Walsh}, it is shown that $S_\qoi(\param)$ is efficiently computed using a singular value decomposition (SVD) of the Jacobian $J_{\param,\qoi}$, i.e., we randomly sample $J_{\param,\qoi}$ and compute the SVDs.
